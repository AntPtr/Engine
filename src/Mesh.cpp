...
Mesh::Mesh(std::vector<Vertex> vert, std::vector<Texture> tex, std::vector<unsigned int> ind)
	: vertices(vert), textures(tex), indices(ind) {
	setupMesh();
}
...
void Mesh::draw(cShaderProg shader) {
	// bind appropriate textures
	unsigned int diffuseNr = 1;
	unsigned int specularNr = 1;
	unsigned int normalNr = 1;
	unsigned int heightNr = 1;
	unsigned int refleNr = 1;
	for (unsigned int i = 0; i < textures.size(); i++)
	{
		...
		else if (name == "texture_reflecion")
